 ...
 * 
 * Modified: @author tkukurin / https://github.com/tkukurin
 * Allows setting limits on panning.
 * TODO handle zoom, fix movement being relative to mouse position.
 * 
 */

OrthographicControls = function ( object, limits, zoomCallback, domElement ) {
	...
	this.zoomCamera = function () {
		...
		if ( _state === STATE.TOUCH_ZOOM_PAN ) {
			...
			var factor = 1.0 + ( _zoomStart.y - _zoomEnd.y ) * _this.zoomSpeed;
		...
		/*if (this.object.zoomCallback)
			_this.object.zoomCallback(_this.object.zoom);
		...
		const outOfBoundsX = _this.limits && Math.abs(objX) >= _this.limits.x * _this.object.zoom;
		const outOfBoundsY = _this.limits && Math.abs(objY) >= _this.limits.y * _this.object.zoom;

		if (outOfBoundsX) {
			_this.object.position.x = _this.limits.x * Math.sign(objX) * _this.object.zoom;
            _this.target.x += _this.limits.x * Math.sign(objX) * _this.object.zoom;
		}

		if (outOfBoundsY) {
			_this.object.position.y = _this.limits.y * Math.sign(objY) * _this.object.zoom;
            _this.target.y += _this.limits.y * Math.sign(objY) * _this.object.zoom;
		}

		console.log(_this.object.zoom);*/
...
OrthographicControls.prototype = Object.create( THREE.EventDispatcher.prototype );
