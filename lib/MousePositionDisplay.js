...
const inf = 30;
const axes = [ buildAxis(vec2(-inf, inf), vec2(-inf, -inf), 0xFFAAAA), 
               buildAxis(vec2(-inf, -inf), vec2(inf, -inf), 0xFFAAAA) ];
...
document.addEventListener('mousemove', (e) => {
    ...
    const roundingFn = val => val; // Math.round, TODO allow snapping to grid
    ...
    const xp = roundingFn(closestX.point.x);

    const oldY = axes[1].geometry.vertices[0].y;
    const yp = roundingFn(closestY.point.y);
    ...
    positionDataElement.innerHTML = "x: " + xp.toFixed(2) + ", y: " + yp.toFixed(2);
});
...
renderer.context.canvas.oncontextmenu = e => e.preventDefault();

var panPosition;
document.addEventListener('mousedown', e => {
    if (e.which === 3) {
        panPosition = new THREE.Vector2(e.pageX, e.pageY);
        return false;
    }
});
...
document.addEventListener('mousemove', e => {
    ...
    const dx = (currentMouse.x - oldMouse.x) * (camera.right - camera.left) / (2 * camera.zoom);
    const dy = (currentMouse.y - oldMouse.y) * (camera.top - camera.bottom) / (2 * camera.zoom);
