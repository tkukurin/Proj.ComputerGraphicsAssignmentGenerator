...
document.addEventListener('a', e => {
        ...
        /*const intersectObjectIndex = intersectionFinder.getIntersections(e.clientX, e.clientY, scene.children)
                .map(o => o.userData)
                .filter(ud => ud)
...
document.addEventListener('a', e => {
        ...
        const objects = intersectionFinder.getIntersections(e.clientX, e.clientY, scene.children);
        var o = [];
        if (objects.length>0) {
        ...
        for(var i in scene.children) {
            ...
            } else if (scene.children[i].userData.verticeIndex) {
                scene.children[i].userData.verticeIndex.color.setHex(0xff0000);
            }
        }
}, false);
