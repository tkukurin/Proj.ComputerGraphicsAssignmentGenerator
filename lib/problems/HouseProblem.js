...
function house() {
    ...
    const base = colorSquare(vec2(0, 1), new Dimensions(w, w));
    const door = colorSquare(vec2(0.5, 0.5), new Dimensions(w / 4, w / 2));
    const window = colorSquare(vec2(-0.5, 1.5), new Dimensions(w / 4, w / 4));
    const top = colorTriangle(vec2(-1, w), vec2(1, w), vec2(0, w + 1));
